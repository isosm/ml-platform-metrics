/*
  ...
  Combines all 8 DORA-for-ML metrics into a single composited health score
  (0–100) using weighted KPIs. Think of it as a credit score for your ML platform.

  Score weights (total = 100):
    25 pts  Deployment Frequency    — are teams shipping regularly?
    20 pts  CI Failure Rate         — is quality gate holding?
    20 pts  Model Drift Rate        — are models staying healthy?
    20 pts  Lead Time               — is feedback loop tight?
    15 pts  MTTR                    — are incidents resolved fast?

  Thresholds are calibrated to DORA "Elite" benchmarks and ML SRE best practice.
  Below 70 = alert. Above 90 = healthy.

  This model has a dbt contract enforced — any schema change requires an
  explicit contract update, protecting downstream consumers (dashboard, Evidence).
*/
...
scored as (
    select
        *,
        ...
        /*
          Component scores (0–100 each), then weighted sum.

          Deployment Frequency (25 pts):
            Elite: ≥ 1 deploy/day     → 100
            High:  ≥ 1/week           → 75
            Medium: ≥ 1/month         → 50
            Low: below                → 25
        */
        ...
        -- CI Failure Rate (20 pts): elite < 15%, low > 45%
        greatest(0, least(100, (1 - ci_failure_rate_7d_avg) * 100))
            as score_ci_quality,

        -- Drift Failure Rate (20 pts): target < 5%
        greatest(0, least(100, (1 - drift_failure_rate_7d_avg) * 100))
            as score_drift,

        -- Lead Time (20 pts): elite < 1 day, low > 7 days
        case
        ...
        -- MTTR (15 pts): elite < 1h, low > 24h
        case
            when avg_mttr_hours = 0     then 100.0  -- no incidents
...
final as (
    select
        ...
        -- Health tier for dashboard colouring
        case
